...
async function show(req, res) {
  try {
    ...
    const populateIng = await Recipe.findById(req.params.id).populate(
      "ingredients"
    ); //populates ingredients from the recipe model
    ...
    }).sort("newIngredient"); //finds all ingredients that are not already in recipe's ingredients array and stores and sorts result
